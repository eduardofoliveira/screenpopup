...
const init = connection => {
  ...
  app.get("/:id", async (req, res) => {
    ...
    chamado.inicio = `${chamado.inicio.getDate()}/${(chamado.inicio.getMonth() + 1)
      .toString()
      .padStart(2, "0")}/${chamado.inicio.getFullYear()} ${chamado.inicio
      .getHours()
      .toString()
      .padStart(2, "0")}:${chamado.inicio
      .getMinutes()
      .toString()
      .padStart(2, "0")}:${chamado.inicio
      .getSeconds()
      ...
      .padStart(2, "0")}`;
    if (chamado.termino) {
      chamado.termino = `${chamado.termino.getDate()}/${(chamado.termino.getMonth() + 1)
        .toString()
        .padStart(2, "0")}/${chamado.termino.getFullYear()} ${chamado.termino
        .getHours()
        .toString()
        .padStart(2, "0")}:${chamado.termino
        .getMinutes()
        .toString()
        .padStart(2, "0")}:${chamado.termino
        .getSeconds()
  ...
  app.put("/:id", async (req, res) => {
    ...
    let [existe] = await connection.query("SELECT * FROM chamado WHERE id = ?", [id]);
    if (existe.length === 1) {
      await connection.query("UPDATE chamado set aberto = 0, comentario = ?, fk_fechado_por = ? where id = ?", [
        chamado.comentario,
        req.session.user.id,
        id
      ]);
